...
def play() :  #Fonction basique qui renvoie une nouvelle position la ou c est possible de maniere aleatoire
    ...
    for key, value in message['state']['board'][pos].items() :
        ...
        if value == True:
            if key == 'S' and tile[0<=a<=48]['N']== True and pos not in [42, 43, 44, 45, 46, 47, 48]:
                print(a)
                new_pos = a
            
            elif key == 'N'  and tile[0<=b<=48]['S']== True and pos not in [0, 1, 2, 3, 4, 5, 6]:
                new_pos = b
                print(b)

            elif key == 'E'  and tile[0<=c<=48]['W']== True and pos not in [6, 13, 20, 27, 34, 41, 48]:
                new_pos = c
                print(c)

            elif key == 'W'  and tile[0<=d<=48]['E']== True and pos not in [0, 7, 14, 21, 28, 35, 42]:
                new_pos = d
                print(d)
